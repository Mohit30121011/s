...
public class PortDAO {
    public List<Port> getAllPorts() {
        ...
        String sql = "SELECT * FROM ports ORDER BY port_id ASC";
    ...
    public Port getPortById(int portId) {
        String sql = "SELECT * FROM ports WHERE port_id = ?";
        try (Connection conn = DBConnectionManager.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, portId);
    ...
    public void updatePort(int portId, String name, String code, String country, double lat, double lng) {
        String sql = "UPDATE ports SET port_name = ?, port_code = ?, country = ?, latitude = ?, longitude = ? WHERE port_id = ?";
        try (Connection conn = DBConnectionManager.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, name);
            ps.setString(2, code);
    ...
    public void deletePort(int portId) {
        String sql = "DELETE FROM ports WHERE port_id = ?";
        try (Connection conn = DBConnectionManager.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, portId);
            ps.executeUpdate();
    ...
    /**
     * DB SP: delete_ports(p_port_id, p_requesting_user_id) - Super Admin only
     */
    ...
    /**
     * DB SP: update_ports(p_port_id, p_requesting_user_id, name, code, country, lat, lng)
     */
    public void updatePort(int portId, int requestingUserId, String name, String code, String country, Double lat, Double lng) {
        ...
        try (Connection conn = DBConnectionManager.getConnection();
            ...
            updatePort(portId, name, code, country, (lat != null ? lat : 0.0), (lng != null ? lng : 0.0));
        }
    }
}
